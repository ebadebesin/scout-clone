...
const Export = ({ filteredSales }) => {
    const exportToCSV = () => {
        const worksheet = XLSX.utils.json_to_sheet(filteredSales.map(item => ({
            ...item,
            PurchaseDate: item.PurchaseDate ? item.PurchaseDate.toLocaleDateString() : 'N/A'
        })))
        const workbook = XLSX.utils.book_new()
        XLSX.utils.book_append_sheet(workbook, worksheet, 'Sales')
        XLSX.writeFile(workbook, 'sales.csv')
    }

    return (
        <Tooltip title="Export your sales to CSV">
