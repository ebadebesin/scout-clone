...
export default function Sales() {
  ...
  const handleSaveChanges = async () => {
    ...
    const updatedSale = {
      ...selectedSale,
      purchasePrice: String(selectedSale.purchasePrice).replace("$", ""), // Ensure purchasePrice is a string
      salePrice: String(selectedSale.salePrice).replace("$", ""), // Ensure salePrice is a string
      soldDate: new Date(selectedSale.soldDate).toISOString(), // Ensure valid date for saving
    };
      ...
      <Box component="main" sx={{ flexGrow: 1, padding: 3, position: "relative" }}>
              ...
              <TableBody>
                {sales.map((item) => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell>
                      {item.soldDate
                        ? format(new Date(item.soldDate), "MMM dd, yyyy")
                        : "Invalid date"}
                    </TableCell>
                    <TableCell>
        ...
        <Modal open={openDialog} onClose={() => setOpenDialog(false)}>
          ...
          >
            ...
            <Box sx={{ display: "flex", justifyContent: "flex-end", marginTop: "20px" }}>
              <Button onClick={() => setOpenDialog(false)} sx={{ marginRight: "10px" }}>
                Cancel
              </Button>
        ...
        <Modal open={editDialogOpen} onClose={() => setEditDialogOpen(false)}>
          ...
          >
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} sm={6}>
                <TextField
                  ...
                  value={
                    selectedSale?.soldDate
                      ? format(new Date(selectedSale?.soldDate), "yyyy-MM-dd")
                      : ""
                  }
                  onChange={(e) =>
            ...
            <Box sx={{ display: "flex", justifyContent: "flex-end", marginTop: "20px" }}>
              <Button onClick={() => setEditDialogOpen(false)} sx={{ marginRight: "10px" }}>
                Cancel
              </Button>
